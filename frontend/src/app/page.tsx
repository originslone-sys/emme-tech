  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-6 md:mb-10">
        <h1 className="text-2xl md:text-3xl font-bold text-white">Editor de Vídeo</h1>
        <p className="text-white/40 mt-1 text-sm">Melhore, corte e monte seus vídeos</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-1 gap-3 md:gap-4">
        {cards.map((card) => (
          <Link
            key={card.href}
            href={card.href}
            className="group flex flex-col md:flex-row items-start gap-3 md:gap-5 p-4 md:p-6 bg-[#111] border border-white/10 rounded-xl hover:border-violet-500/40 transition-all"
          >
            <div className="text-2xl md:text-3xl md:mt-0.5">{card.icon}</div>
            <div className="flex-1 min-w-0">
              <h2 className="text-sm md:text-base font-semibold text-white leading-snug">{card.title}</h2>
              <p className="text-xs md:text-sm text-white/40 mt-1 hidden md:block">{card.description}</p>
            </div>
            <span className="hidden md:block text-violet-400 group-hover:text-violet-300 text-sm self-center transition-colors">→</span>
          </Link>
        ))}
      </div>
    </div>
