...
def _download_generic(url: str, vid: str, out_tmpl: str) -> str:
    ...
    try:
        ...
        if "429" in msg or "too many requests" in msg.lower():
            raise RuntimeError(
                "O Google Drive atingiu o limite de downloads deste arquivo (erro "
                "429). Isso é uma cota do próprio arquivo no Drive e não some "
                "rápido. Use outro link ou envie o vídeo pelo upload."
            )
