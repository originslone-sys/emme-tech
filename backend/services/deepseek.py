...
# ---------- Roteiro de curiosidades para automação TikTok ----------

_SUBTHEMES = [
    "espaço", "corpo humano", "oceano", "história", "animais",
    "psicologia", "comida", "tecnologia",
]

_SYSTEM_CURIOSITY = (
    "Você é um roteirista especialista em conteúdo viral de curiosidades para TikTok. "
    "Cria vídeos de 60 segundos no estilo 'Jornada da Curiosidade': "
    "Gancho → Conflito → Fato Inusitado → Consequência → CTA. "
    "Linguagem casual, como se fosse um amigo contando algo incrível. "
    "Frases curtas e impactantes. Sem palavras difíceis ou rebuscadas. "
...
def _build_curiosity_prompt(used_keys: list[str]) -> str:
    import random
    subtheme = random.choice(_SUBTHEMES)
    used_str = ", ".join(used_keys[-30:]) if used_keys else "nenhum ainda"
    return (
        f"Crie um roteiro de TikTok de curiosidades sobre o sub-tema: **{subtheme}**.\n\n"
        f"FATOS JÁ USADOS (PROIBIDO repetir qualquer um): {used_str}\n\n"
        "Estrutura OBRIGATÓRIA — Jornada da Curiosidade (20 cenas, ~60s total):\n"
        "1. GANCHO (cenas 1-2): Pergunta chocante ou afirmação impossível. 'Você sabia que...?' "
        "Cria o impulso de não parar de assistir. 4-6s total.\n"
        "2. CONFLITO (cenas 3-4): 'Mas ninguém te conta que...' — cria tensão e suspense. 6-8s total.\n"
        "3. FATO INUSITADO (cenas 5-16): O coração do vídeo. Explica a curiosidade em detalhes "
        "fascinantes com exemplos concretos e visuais marcantes. ~36s total.\n"
        "4. CONSEQUÊNCIA (cenas 17-18): 'E o mais incrível é...' — a revelação final que "
        "recontextualiza tudo. 6s total.\n"
        "5. CTA (cenas 19-20): Chamada pra ação concreta e específica. 6s total.\n\n"
        "REGRAS OBRIGATÓRIAS:\n"
        "- 'text' na tela: NO MÁXIMO 5 palavras. Sem pontuação. Só letras e espaços.\n"
        "- 'narration': 1 frase curta e casual (como falando pra um amigo). Sem palavras difíceis.\n"
        "- 'visual_query': em inglês, concreto e filmável em banco de vídeo stock "
        "(ex: 'octopus swimming ocean closeup'). Sem conceitos abstratos.\n"
        "- 'fact_key': slug único do fato em português-kebab-case "
        "(ex: 'polvo-tem-3-coracoes', 'lua-se-afasta-terra'). DEVE ser diferente dos usados.\n"
        "- 'duration': entre 2 e 4 segundos por cena.\n"
        "- Total: exatamente 20 cenas.\n\n"
        "Responda APENAS em JSON válido:\n"
        '{"title": "título chamativo até 60 chars", '
        '"description": "legenda com emojis e hashtags (120-150 chars)", '
        '"tags": ["tag1", "tag2", "tag3", "tag4", "tag5"], '
        '"music_mood": "energetic", '
        f'"fact_key": "slug-unico-do-fato", '
        f'"subtheme": "{subtheme}", '
        '"hook_text": "frase de impacto do gancho", '
        '"emphasis_word": "palavra mais importante do video", '
        '"cta_type": "salvar", '
        '"scenes": [{"text": "texto curto", "narration": "frase falada", '
        '"visual_query": "english stock query", "duration": 3}]}'
    )


async def generate_curiosity_script(used_keys: list[str] | None = None) -> dict:
    """Gera roteiro de curiosidade com anti-repetição via fact_key."""
    used_keys = used_keys or []
    payload = {
        "model": "deepseek-chat",
        "messages": [
            {"role": "system", "content": _SYSTEM_CURIOSITY},
            {"role": "user", "content": _build_curiosity_prompt(used_keys)},
        ],
        "response_format": {"type": "json_object"},
        "temperature": 0.9,
    }
    ...
    fact_key = str(data.get("fact_key", "")).strip().lower()[:60] or "fato-desconhecido"
    # Garante unicidade básica se a IA repetiu
    if fact_key in used_keys:
        import uuid as _uuid
        fact_key = f"{fact_key}-{_uuid.uuid4().hex[:6]}"

    return {
        "title": str(data.get("title", "")).strip()[:80] or "Curiosidade Incrível",
        "description": str(data.get("description", "")).strip(),
        "tags": [str(t).strip() for t in data.get("tags", []) if str(t).strip()][:10],
        "music_mood": mood,
        "fact_key": fact_key,
        "subtheme": str(data.get("subtheme", "")).strip(),
        "hook_text": str(data.get("hook_text", "")).strip(),
        "emphasis_word": str(data.get("emphasis_word", "")).strip(),
        "cta_type": str(data.get("cta_type", "salvar")).strip(),
        "scenes": scenes,
    }
